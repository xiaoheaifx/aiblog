...
export default function App() {
  ...
  // 调用后端 API 记录阅读量（IP 去重，24 小时内同 IP 不重复计数）
  try {
    const response = await fetch(`/api/post/view?postId=${id}`, { method: 'POST' });
    const data = await response.json();

    if (response.ok && data.views !== undefined) {
      // 更新该文章的 views 字段为后端返回的真实值
      setPosts(prevPosts =>
        prevPosts.map(p =>
      ...
      console.warn('Failed to record view:', data);
    }
  } catch (err) {
    ...
    <div className={`min-h-screen flex flex-col font-sans transition-colors duration-200 selection:bg-indigo-500/10 selection:text-indigo-600 ${
      ...
      <header className="sticky top-0 z-40 bg-white/95 dark:bg-[#090d16]/90 backdrop-blur-md border-b border-slate-200 dark:border-slate-800/80 transition-all shadow-[0_4px_30px_rgba(0,0,0,0.015)]">
        <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 h-20 flex items-center justify-between gap-4">
          ...
          {/* Left brand logo and navigation menu */}
        ...
        <div className="flex-1 min-w-0 space-y-8">
          ...
          {currentTab === 'home' && !selectedPostId && (
            <div className="space-y-8 animate-fadeIn">
              ...
              {loadingPosts ? (
                <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 p-8 rounded-2xl text-center">
          ...
          {currentTab === 'articles' && !selectedPostId && (
            <div className="space-y-6 animate-fadeIn">
              ...
              {loadingPosts ? (
                <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-8 rounded-xl shadow-sm text-center">
                  <div className="animate-pulse text-slate-500">加载中...</div>
                </div>
          ...
          {/* ARTICLE DETAIL VIEW (保持不变，仅引用 posts 和 comments) */}
          {selectedPostId && currentPost && (
            <div className="space-y-4 animate-fadeIn">
              ...
              <article className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm rounded-xl overflow-hidden p-4 sm:p-6">
                <div className="space-y-3">
                  ...
                  <div className="flex flex-wrap items-center gap-4 text-xs font-mono text-slate-400 border-b border-slate-100 dark:border-slate-800 pb-4">
                    ...
                    <span className="flex items-center gap-1"><Heart className="w-3.5 h-3.5 text-rose-500 fill-rose-500/10" />{currentPost.likes}</span>
                  </div>
                </div>
                ...
                <div className="mt-8 pt-6 border-t border-slate-150 dark:border-slate-800/80 flex flex-col sm:flex-row items-center justify-between gap-4">
                  ...
                  <div className="flex flex-wrap items-center gap-1.5">
                    ...
                    {/* X, WeChat, Weibo, Facebook, Instagram, Youtube, Telegram, Link */}
                    <button onClick={() => handleShare(currentPost.title, 'x')} className="p-1.5 rounded-full bg-black text-white hover:bg-slate-900 border border-slate-800/20 transition-all shadow-sm transform hover:-translate-y-0.5"><svg viewBox="0 0 24 24" fill="currentColor" className="w-3.5 h-3.5"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg></button>
                    <button onClick={() => handleShare(currentPost.title, 'wechat')} className="p-1.5 rounded-full bg-[#07C160] text-white hover:bg-[#06af54] transition-all shadow-sm transform hover:-translate-y-0.5"><svg viewBox="0 0 24 24" fill="currentColor" className="w-3.5 h-3.5"><path d="M8.2 14.2c-.3 0-.6-.2-.6-.5 0-.3.3-.5.6-.5.3 0 .6.2.6.5 0 .3-.3.5-.6.5zm3.8 0c-.3 0-.6-.2-.6-.5 0-.3.3-.5.6-.5.3 0 .6.2.6.5 0 .3-.3.5-.6.5zm.9-8.3C10.9 5.3 8.8 5 6.6 5 2.8 5 0 7.6 0 10.9c0 1.8 1 3.4 2.5 4.6l-.6 1.9 2.1-1.1c.8.2 1.6.3 2.5.3h.3c-.2-.6-.3-1.2-.3-1.9 0-3.3 2.9-6 6.5-6h.4c-.9-1.2-2.1-2.1-3.5-2.7zm5.1 4c-3.1 0-5.7 2.2-5.7 4.9s2.6 4.9 5.7 4.9c.7 0 1.5-.1 2.2-.4l1.6.8-.5-1.5c1.1-1 1.9-2.3 1.9-3.8.1-2.7-2.5-4.9-5.7-4.9zm-2.1 4c-.3 0-.5-.2-.5-.5 0-.3.2-.5.5-.5s.5.2.5.5c0 .3-.2.5-.5.5zm4.2 0c-.3 0-.5-.2-.5-.5 0-.3.2-.5.5-.5s.5.2.5.5c0 .3-.2.5-.5.5z"/></svg></button>
                    <button onClick={() => handleShare(currentPost.title, 'weibo')} className="p-1.5 rounded-full bg-[#DF2029] text-white hover:bg-[#c51c24] transition-all shadow-sm transform hover:-translate-y-0.5"><svg viewBox="0 0 24 24" fill="currentColor" className="w-3.5 h-3.5"><path d="M9.704 14.242c1.948.337 3.738-.415 4-.1.261.315-1.077 1.993-2.946 2.316C8.89 16.782 7.11 16.14 6.85 15.825c-.261-.315 1.076-1.993 2.854-2.316zm6.386-4.526c.23-.21.246-.532.036-.762a.544.544 0 0 0-.762-.036 6.57 6.57 0 0 0-1.782 2.392c-.13.3 0 .653.303.782.3.13.653 0 .782-.3a5.38 5.38 0 0 1 1.423-2.076zm2.222-2.13c.234-.206.257-.557.05-.791a.564.564 0 0 0-.79-.05 10.3 10.3 0 0 0-3.139 3.916.551.551 0 0 0 .307.725.556.556 0 0 0 .723-.307 9.17 9.17 0 0 1 2.85-3.493zM22.9 8.281a8.68 8.68 0 0 0-3.3-3.003h-.002c-.3-.153-.667-.037-.82.261-.153.3-.037.667.261.82 1.348.687 2.15 1.767 2.385 3.208.06.326.37.535.696.475.326-.06.535-.371.475-.697a4.9 4.9 0 0 0-.695-1.064zM10.867 7.009c-4.493-.314-8.158 1.96-8.183 5.078-.025 3.118 3.593 5.908 8.086 6.223 4.494.314 8.16-1.96 8.185-5.078.025-3.118-3.593-5.908-8.088-6.223zm-.088 9.535c-2.482-.174-4.432-1.731-4.355-3.483.076-1.751 2.152-3.033 4.633-2.86 2.48.173 4.43 1.732 4.353 3.483s-2.15 3.033-4.631 2.86z"/></svg></button>
                    <button onClick={() => handleShare(currentPost.title, 'facebook')} className="p-1.5 rounded-full bg-[#1877F2] text-white hover:bg-[#166fe5] transition-all shadow-sm transform hover:-translate-y-0.5"><Facebook className="w-3.5 h-3.5" /></button>
                    <button onClick={() => handleShare(currentPost.title, 'instagram')} className="p-1.5 rounded-full bg-gradient-to-tr from-[#f9ce34] via-[#ee2a7b] to-[#6228d7] text-white hover:brightness-110 transition-all shadow-sm transform hover:-translate-y-0.5"><Instagram className="w-3.5 h-3.5" /></button>
                    <button onClick={() => handleShare(currentPost.title, 'youtube')} className="p-1.5 rounded-full bg-[#FF0000] text-white hover:bg-[#cc0000] transition-all shadow-sm transform hover:-translate-y-0.5"><Youtube className="w-3.5 h-3.5" /></button>
                    <button onClick={() => handleShare(currentPost.title, 'telegram')} className="p-1.5 rounded-full bg-[#0088cc] text-white hover:bg-[#0077b5] transition-all shadow-sm transform hover:-translate-y-0.5"><Send className="w-3.5 h-3.5" /></button>
                    <button onClick={() => handleShare(currentPost.title, 'link')} className="p-1.5 rounded-lg bg-indigo-600 hover:bg-indigo-505 text-white font-bold text-[10px] uppercase tracking-wider px-2.5 py-1.5 transition-all shadow-sm transform hover:-translate-y-0.5"><span>LINK</span></button>
                  </div>
                </div>
              </article>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {prevPost ? (
                  <div onClick={() => handleSelectPost(prevPost.id)} className="p-4 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-850 hover:border-slate-200 dark:hover:border-slate-750 shadow-sm rounded-xl cursor-pointer hover:shadow-md transition-all text-left space-y-1 block">
                ...
                {nextPost ? (
                  <div onClick={() => handleSelectPost(nextPost.id)} className="p-4 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-850 hover:border-slate-200 dark:hover:border-slate-750 shadow-sm rounded-xl cursor-pointer hover:shadow-md transition-all text-right space-y-1 block">
              ...
              <section className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-850 shadow-md rounded-2xl p-5 sm:p-8 space-y-6">
                ...
                <form onSubmit={(e) => handlePublishComment(e, currentPost.id)} className="space-y-4">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div><label className="text-xs text-slate-400 block mb-1">{translations[locale].commentNickname}</label><input type="text" required placeholder="e.g. CodeExplorer" value={newNickname} onChange={e => setNewNickname(e.target.value)} className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-805 text-sm rounded-xl p-2.5 outline-none focus:ring-2 focus:ring-sky-500/50 text-slate-800 dark:text-slate-100 transition-all font-medium" /></div>
                    <div><label className="text-xs text-slate-400 block mb-1">{translations[locale].commentEmail}</label><input type="email" required placeholder="e.g. public@domain.io" value={newEmail} onChange={e => setNewEmail(e.target.value)} className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-805 text-sm rounded-xl p-2.5 outline-none focus:ring-2 focus:ring-sky-500/50 text-slate-800 dark:text-slate-100 transition-all font-medium" /></div>
                  </div>
                  <div className="space-y-1">
                    <label className="text-xs text-slate-400 block mb-1">{translations[locale].commentContent}</label>
                    <WysiwygEditor locale={locale} value={newCommentVal} onChange={val => setNewCommentVal(val)} placeholder={translations[locale].commentPlaceholder} />
                  </div>
                  <button type="submit" className="bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-sm px-6 py-2.5 rounded-xl transition-all shadow-md flex items-center gap-1.5 hover:shadow-emerald-500/20"><Send className="w-4 h-4" /><span>{translations[locale].commentSubmit}</span></button>
                </form>

                <div className="divide-y divide-slate-100 dark:divide-slate-800 pt-4 space-y-4">
                  {comments.filter(c => c.postId === currentPost.id && (!c.isHidden || isAdmin)).map(comment => (
                    <div key={comment.id} className={`pt-4 flex gap-3.5 items-start ${comment.isHidden ? 'opacity-60 bg-red-500/5 dark:bg-red-500/10 p-2.5 rounded-xl border border-red-500/10' : ''}`}>
                      <img src={comment.avatar} alt="Avatar" className="w-10 h-10 rounded-xl bg-slate-50 border border-slate-200 dark:border-slate-750 shrink-0 shadow-inner" />
                      <div className="flex-1 min-w-0 space-y-1.5">
                        <div className="flex items-center justify-between">
                          <div><strong className="text-slate-800 dark:text-slate-100 text-sm font-semibold">{comment.nickname}</strong>{comment.replyTo && <span className="text-xs text-slate-300 bg-sky-500/10 px-1.5 py-0.5 rounded mx-2">{locale === 'zh' ? '回复' : 'Reply to'} @{comment.replyTo}</span>}{comment.isHidden && <span className="text-[10px] text-red-500 bg-red-500/10 dark:bg-red-500/20 px-1.5 py-0.5 rounded mx-2 font-bold select-none">{locale === 'zh' ? '已隐藏' : 'Hidden'}</span>}</div>
                          <span className="text-[10px] text-slate-400 font-mono">{comment.date}</span>
                        </div>
                        <div className="text-slate-600 dark:text-slate-300 text-xs sm:text-sm leading-relaxed bg-slate-50 dark:bg-slate-950/40 p-3 rounded-xl border border-slate-100 dark:border-slate-850 prose dark:prose-invert" dangerouslySetInnerHTML={{ __html: comment.content }} />
                        <div className="flex justify-end">
                          {replyingToCommentId === comment.id ? (
                            <button onClick={() => setReplyingToCommentId(null)} className="text-xs text-slate-500 cursor-pointer hover:text-slate-700 font-bold">{translations[locale].commentCancelReply}</button>
                          ) : (
                            <button onClick={() => { setReplyingToCommentId(comment.id); setReplyNickname(''); setReplyEmail(''); setReplyCommentVal(''); }} className="text-xs text-indigo-500 hover:text-indigo-400 font-bold hover:underline">{translations[locale].commentReply}</button>
                          )}
                        </div>
                        {replyingToCommentId === comment.id && (
                          <form onSubmit={(e) => handlePublishReply(e, comment.id, comment.nickname)} className="bg-slate-50 dark:bg-slate-950/60 p-4 border border-slate-200 dark:border-slate-800 rounded-xl space-y-3.5 mt-2 animate-fadeIn">
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
                              <div><label className="text-xs text-slate-400 block mb-1">{translations[locale].commentNickname}</label><input type="text" required placeholder="Your Name..." value={replyNickname} onChange={e => setReplyNickname(e.target.value)} className="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-750 text-xs rounded-lg p-2 outline-none focus:ring-1 focus:ring-sky-500 text-slate-800 dark:text-slate-100" /></div>
                              <div><label className="text-xs text-slate-400 block mb-1">{translations[locale].commentEmail}</label><input type="email" required placeholder="public@domain.com" value={replyEmail} onChange={e => setReplyEmail(e.target.value)} className="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-750 text-xs rounded-lg p-2 outline-none focus:ring-1 focus:ring-sky-500 text-slate-800 dark:text-slate-100" /></div>
                            </div>
                            <div className="space-y-1"><label className="text-xs text-slate-400 block mb-1">{translations[locale].commentContent}</label><WysiwygEditor locale={locale} value={replyCommentVal} onChange={v => setReplyCommentVal(v)} placeholder={translations[locale].commentPlaceholder} /></div>
                            <div className="flex gap-2"><button type="submit" className="bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs px-4 py-2 rounded-lg transition-all shadow">{translations[locale].commentSubmit}</button><button type="button" onClick={() => setReplyingToCommentId(null)} className="bg-slate-200 dark:bg-slate-800 text-slate-600 dark:text-slate-350 text-xs px-3.5 py-2 rounded-lg">{translations[locale].commentCancelReply}</button></div>
                          </form>
                        )}
                      </div>
                    </div>
                  ))}
                  {comments.filter(c => c.postId === currentPost.id).length === 0 && <p className="text-xs text-slate-450 dark:text-slate-500 pt-2 italic text-left">{locale === 'zh' ? '暂无留言。撰写上面的表单，成为第一个发表高见的人！' : 'Be the first to post a thought above!'}</p>}
                </div>
              </section>
            </div>
          )}
      ...
      <footer className="mt-auto border-t border-slate-150 dark:border-slate-800 bg-white/40 dark:bg-slate-950/40 text-slate-400 py-6 text-center text-xs space-y-1.5 shadow-inner">
        <p className="font-medium text-slate-655 dark:text-slate-400">{translations[locale].footerText}</p>
      ...
      {showLoginModal && (
        <div className="fixed inset-0 z-50 bg-slate-900/40 dark:bg-[#070b13]/85 backdrop-blur-sm flex items-center justify-center p-4 text-slate-800 dark:text-slate-100 font-sans animate-fadeIn">
          <form onSubmit={handleLogin} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl w-full max-w-sm p-6 shadow-2xl relative space-y-4 animate-scaleUp">
            <button type="button" onClick={() => setShowLoginModal(false)} className="absolute top-4 right-4 text-slate-400 hover:text-slate-600 dark:hover:text-white transition-colors"><XIcon className="w-5 h-5" /></button>
            <div className="flex items-center gap-2"><Sparkles className="w-5 h-5 text-indigo-500 dark:text-sky-400 animate-pulse" /><strong className="text-base text-slate-900 dark:text-slate-100 font-bold">{translations[locale].adminLoginTitle}</strong></div>
            <div className="space-y-3">
              <div><label className="text-xs text-slate-500 dark:text-slate-400 block mb-1">{locale === 'zh' ? '管理账号' : 'Username'}</label><input type="text" required placeholder={locale === 'zh' ? "请输入管理账号" : "Username"} value={adminUsername} onChange={e => setAdminUsername(e.target.value)} className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-250 dark:border-slate-750 focus:border-indigo-550 rounded-lg p-2.5 outline-none text-sm font-mono text-slate-900 dark:text-white transition-all shadow-inner" /></div>
              <div><label className="text-xs text-slate-500 dark:text-slate-400 block mb-1">{locale === 'zh' ? '密码' : 'Password'}</label><input type="password" required placeholder={locale === 'zh' ? "请输入密码" : "Password"} value={adminPassword} onChange={e => setAdminPassword(e.target.value)} className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-250 dark:border-slate-750 focus:border-indigo-550 rounded-lg p-2.5 outline-none text-sm font-mono text-slate-900 dark:text-white transition-all shadow-inner" /></div>
            </div>
            <div className="flex gap-2.5 pt-1">
              <button type="submit" className="flex-1 bg-gradient-to-r from-sky-500 to-indigo-500 text-white font-bold text-xs py-2.5 rounded-lg hover:brightness-110 shadow-md transition-all active:scale-95">{translations[locale].submit}</button>
              <button type="button" onClick={() => setShowLoginModal(false)} className="bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-750 text-slate-700 dark:text-slate-300 font-bold text-xs px-4 rounded-lg border border-slate-200 dark:border-transparent transition-all">{translations[locale].cancel}</button>
            </div>
          </form>
        </div>
      )}
